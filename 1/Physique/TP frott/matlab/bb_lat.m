...
input.tableLabel = 'mesurebb';


latex = latexTable(input);
fid=fopen('bb_r1.tex','w');
...
input.tableLabel = 'anglebb';

latex = latexTable(input);
fid=fopen('bb_r2.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
...
input.tableLabel = 'v-abb';


latex = latexTable(input);
fid=fopen('bb_r3.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
...
input.tableLabel = 'coeffbb';


latex = latexTable(input);
fid=fopen('bb_r5.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
