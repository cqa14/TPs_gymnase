...
input.tableLabel = 'mesurepp';


latex = latexTable(input);
fid=fopen('pp_r1.tex','w');
...
input.tableLabel = 'anglepp';

latex = latexTable(input);
fid=fopen('pp_r2.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
...
input.tableLabel = 'v-app';


latex = latexTable(input);
fid=fopen('pp_r3.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
...
input.tableLabel = 'coeffpp';


latex = latexTable(input);
fid=fopen('pp_r5.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
