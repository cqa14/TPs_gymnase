...
input.tableLabel = 'mesurebp';


latex = latexTable(input);
fid=fopen('bp_r1.tex','w');
...
input.tableLabel = 'anglebp';

latex = latexTable(input);
fid=fopen('bp_r2.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
...
input.tableLabel = 'v-abp';


latex = latexTable(input);
fid=fopen('bp_r3.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
...
input.tableLabel = 'coeffbp';


latex = latexTable(input);
fid=fopen('bp_r5.tex','w');
[nrows,ncols] = size(latex);
for row = 1:nrows
    fprintf(fid,'%s\n',latex{row,:});
end
